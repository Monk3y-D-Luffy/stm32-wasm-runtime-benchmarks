...
/* ===== Host API per Wasm: toggle senza read-modify ===== */
...
static void run_wasm_toggle_forever(void) {
    ...
    IM3Module mod = NULL;
    M3Result r = m3_ParseModule(env, &mod, (const uint8_t*)toggle_wasm, toggle_wasm_len);
    if (r) while(1){}
    ...
    // Non ritorna: genera onda continua su PA5
    (void)m3_CallV(f);
    while (1) {} // safety
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
      run_wasm_toggle_forever();  // misura con oscilloscopio su PA5
  while (1)
  {
    /* USER CODE END WHILE */
